#!/usr/bin/env python3
"""Normalize gdelt-doc-search artifacts into a local signal-plane SQLite file."""
...
def public_gap_hints(signals: list[dict[str, Any]]) -> list[str]:
    ...
    hints: list[str] = []
    missing_title = sum(1 for signal in signals if not maybe_text(signal.get("title")))
    missing_time = sum(1 for signal in signals if not maybe_text(signal.get("published_at_utc")))
    missing_url = sum(1 for signal in signals if not maybe_text(signal.get("url")))
    if missing_title:
...
def public_challenge_hints() -> list[str]:
    return ["GDELT doc results can overrepresent repeated pickup around the same narrative."]
...
def build_signals(
    ...
    if not isinstance(articles, list):
        warnings.append({"code": "missing-articles", "message": "Expected payload.articles to be a list."})
        return [], warnings
    if max_records > 0 and len(articles) > max_records:
        warnings.append(
            {"code": "max-records-truncated", "message": f"Truncated GDELT articles from {len(articles)} to {max_records}."}
        )
        articles = articles[:max_records]
    query_text = maybe_text(query_text_override) or maybe_text(payload.get("query"))
    ...
    for index, article in enumerate(articles):
        ...
        signal_id = "sig-" + stable_hash(run_id, round_id, SOURCE_SKILL, artifact_sha256, index, title, url)[:16]
        signals.append(
            {
                "signal_id": signal_id,
                "run_id": run_id,
                ...
                "metadata_json": json.dumps({"domain": domain}, ensure_ascii=True, sort_keys=True),
    ...
    if not signals:
        warnings.append({"code": "no-signals", "message": "No GDELT articles produced normalized signals."})
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Normalize gdelt-doc-search artifacts into a local signal-plane SQLite file.")
