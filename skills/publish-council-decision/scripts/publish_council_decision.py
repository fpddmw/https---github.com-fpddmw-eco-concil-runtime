...
from eco_council_runtime.objects.council import store_decision_trace_records  # noqa: E402
...
def publish_council_decision_skill(
    ...
    if not isinstance(draft_payload, dict):
        ...
        return {
            ...
            "canonical_ids": [],
            "warnings": warnings,
            "board_handoff": {"candidate_ids": [], "evidence_refs": [], "gap_hints": [warnings[0]["message"]], "challenge_hints": [], "suggested_next_skills": ["draft-council-decision"]},
        }
    ...
    if warnings:
        return {
            ...
            "canonical_ids": [],
            "warnings": warnings,
            "board_handoff": {"candidate_ids": [], "evidence_refs": [], "gap_hints": [item["message"] for item in warnings], "challenge_hints": [], "suggested_next_skills": ["draft-council-decision"]},
        }
    ...
    if publication_readiness == "ready" and not skip_report_check:
        ...
        if warnings:
            return {
                ...
                "warnings": warnings,
                "board_handoff": {"candidate_ids": [maybe_text(draft_payload.get("decision_id"))] if maybe_text(draft_payload.get("decision_id")) else [], "evidence_refs": [], "gap_hints": [item["message"] for item in warnings], "challenge_hints": [], "suggested_next_skills": ["publish-expert-report"]},
            }
    ...
    if isinstance(existing, dict):
        if existing == canonical_payload:
            ...
            return {
                ...
                "canonical_ids": [maybe_text(draft_payload.get("decision_id"))] if maybe_text(draft_payload.get("decision_id")) else [],
                "warnings": warnings,
                "board_handoff": {"candidate_ids": [maybe_text(draft_payload.get("decision_id"))] if maybe_text(draft_payload.get("decision_id")) else [], "evidence_refs": [], "gap_hints": [warnings[0]["message"]], "challenge_hints": [], "suggested_next_skills": ["publish-council-decision"]},
            }
