---
name: fetch-regulationsgov-comment-detail
...
```bash
python3 scripts/fetch_regulationsgov_comment_detail.py check-config --pretty
```
...
## Script
- `scripts/fetch_regulationsgov_comment_detail.py`

## OpenClaw Invocation Compatibility
- Keep trigger metadata in `name`, `description`, and `agents/openai.yaml`.
- Invoke with `$fetch-regulationsgov-comment-detail`.
- Keep skill atomic: one invocation consumes one ID set.
- Upstream orchestration can provide IDs from `$fetch-regulationsgov-comments` output.
...
```text
Use $fetch-regulationsgov-comment-detail.
Run:
python3 scripts/fetch_regulationsgov_comment_detail.py fetch \
