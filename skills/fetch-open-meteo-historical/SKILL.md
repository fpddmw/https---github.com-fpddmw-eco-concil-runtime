---
name: fetch-open-meteo-historical
...
```bash
python3 scripts/fetch_open_meteo_historical.py check-config --pretty
```

2. Dry-run the request plan first.

```bash
python3 scripts/fetch_open_meteo_historical.py fetch \
...
python3 scripts/fetch_open_meteo_historical.py fetch \
  ...
  --model era5 \
  --timezone GMT \
  --output ./runs/manual-fetch-artifacts/open-meteo/open-meteo-fetch.json \
  --log-level INFO \
  --log-file ./logs/fetch-open-meteo-historical.log \
  --pretty
```
...
```text
Use $fetch-open-meteo-historical.
Run:
python3 scripts/fetch_open_meteo_historical.py fetch \
