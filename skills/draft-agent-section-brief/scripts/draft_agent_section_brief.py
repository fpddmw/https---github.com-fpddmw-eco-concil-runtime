...
def build_brief(args: argparse.Namespace) -> dict[str, Any]:
    ...
    brief = {
        ...
        "theme_ids": unique_texts([maybe_text(review.get("theme_id")) for review in reviews]),
        "basis_object_ids": basis_ids,
        "sufficiency_review_ids": unique_texts([maybe_text(review.get("review_id")) for review in reviews]),
        "lineage": unique_texts([*basis_ids, *supported_slots, *unsupported_slots, *[maybe_text(review.get("review_id")) for review in reviews]]),
        "provenance": {
    ...
    wrapper = {
        ...
        "provenance": {"source_skill": SKILL_NAME, "decision_source": maybe_text(args.agent_role)},
    }
