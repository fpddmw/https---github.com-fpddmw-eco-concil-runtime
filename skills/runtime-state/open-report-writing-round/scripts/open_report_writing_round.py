...
def build_coordination_context(
    *,
    ...
    round_internal_phases: list[str],
    agent_responsibility_boundaries: list[str],
    theme_progress_review_refs: list[str],
) -> dict[str, Any]:
    return {
        ...
        "program_id": maybe_text(program_id),
        "round_title": maybe_text(round_title),
        "round_subtitle_question": maybe_text(round_subtitle_question),
        "round_category": maybe_text(round_category) or "reporting",
        "round_brief_id": maybe_text(round_brief_id),
        "active_theme_ids": unique_texts(active_theme_ids),
        "round_internal_phases": unique_texts(round_internal_phases),
        "agent_responsibility_boundaries": unique_texts(agent_responsibility_boundaries),
        "theme_progress_review_refs": unique_texts(theme_progress_review_refs),
    ...
    *,
    ...
    return {
        ...
        "inputs": {
            ...
            "program_id": maybe_text(context.get("program_id")),
            "round_brief_id": maybe_text(context.get("round_brief_id")),
            "active_theme_ids": text_values(context.get("active_theme_ids")),
            "theme_progress_review_refs": text_values(context.get("theme_progress_review_refs")),
...
def open_report_writing_round_skill(
    *,
    ...
    theme_progress_review_refs: list[str] | None = None,
    ...
    if program_round_brief:
        requested_program_id = requested_program_id or maybe_text(program_round_brief.get("program_id"))
        ...
        active_theme_ids = unique_texts(
            [*(active_theme_ids or []), *text_values(program_round_brief.get("active_theme_ids"))]
        )
        round_internal_phases = unique_texts(
            [*(round_internal_phases or []), *text_values(program_round_brief.get("round_internal_phases"))]
        )
    ...
    theme_progress_review_refs = unique_texts(
        [
            *(theme_progress_review_refs or []),
            *request_payload_text_list(transition_request, "theme_progress_review_refs"),
    ...
    with locked_board(board_file):
        ...
        context = build_coordination_context(
            ...
            theme_progress_review_refs=theme_progress_review_refs,
        )
        ...
        write_summary = commit_board_mutation(
            ...
            event_payload={
                ...
                "round_mode": ROUND_MODE,
                "program_id": requested_program_id,
                "round_title": maybe_text(context.get("round_title")),
                "round_subtitle_question": maybe_text(context.get("round_subtitle_question")),
                "round_category": maybe_text(context.get("round_category")),
                "round_brief_id": maybe_text(context.get("round_brief_id")),
                "active_theme_ids": text_values(context.get("active_theme_ids")),
                "round_internal_phases": text_values(context.get("round_internal_phases")),
                "agent_responsibility_boundaries": text_values(context.get("agent_responsibility_boundaries")),
                "theme_progress_review_refs": text_values(context.get("theme_progress_review_refs")),
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--theme-progress-review-ref", action="append", default=[])
...
def main() -> int:
    ...
    payload = open_report_writing_round_skill(
        ...
        theme_progress_review_refs=args.theme_progress_review_ref,
