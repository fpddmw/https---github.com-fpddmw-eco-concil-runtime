#!/usr/bin/env python3
"""Normalize gdelt-mentions-fetch manifests into public signal-plane rows."""
...
SKILL_NAME = "eco-normalize-gdelt-mentions-public-signals"
...
def build_signals(payload: Any, run_id: str, round_id: str, artifact_file: Path, artifact_sha256: str) -> tuple[list[dict[str, Any]], list[dict[str, str]]]:
    ...
    if not signals:
        warnings.append({"code": "no-signals", "message": "No GDELT mentions downloads produced normalized rows."})
    return signals, warnings


def normalize_gdelt_mentions(run_dir: str, run_id: str, round_id: str, artifact_path: str, db_path: str) -> dict[str, Any]:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Normalize gdelt-mentions-fetch manifests into public signal-plane rows.")
