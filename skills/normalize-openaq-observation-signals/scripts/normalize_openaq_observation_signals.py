...
SKILL_NAME = "normalize-openaq-observation-signals"
...
def build_signals(
    ...
    if isinstance(payload, dict) and isinstance(payload.get("results"), list):
        ...
        fetch_contract = payload.get("fetch_contract") if isinstance(payload.get("fetch_contract"), dict) else {}
        operation_kind = maybe_text(payload.get("operation_kind"))
        ...
        result_payload = result.get("data") if isinstance(result.get("data"), dict) else {}
        fetch_contract = payload.get("fetch_contract") if isinstance(payload.get("fetch_contract"), dict) else {}
        operation_kind = maybe_text(payload.get("operation_kind")) or maybe_text(payload.get("source_mode_selected"))
    else:
        result_payload = {}
        fetch_contract = {}
        operation_kind = ""
    ...
    for index, item in enumerate(results):
        ...
        if operation_kind:
            quality_flags.append(operation_kind)
        if maybe_text(fetch_contract.get("operation_kind")):
            quality_flags.append(maybe_text(fetch_contract.get("operation_kind")))
        signal_id = "sig-" + stable_hash(run_id, round_id, SOURCE_SKILL, artifact_sha256, index, metric, value)[:16]
        observed_at = maybe_text(item.get("date", {}).get("utc") if isinstance(item.get("date"), dict) else item.get("datetime"))
        signals.append(
            {
                "signal_id": signal_id,
                "run_id": run_id,
                ...
                "metadata_json": json.dumps(
                    {
                        ...
                        "coverage_limitations": [
                            "OpenAQ rows reflect provider and sensor availability for the requested endpoint, location, parameter, and time window.",
                            "Provider-normalized values are evidence inputs, not conclusions about policy adequacy or community exposure.",
                        ],
                    },
                    ensure_ascii=True,
                    sort_keys=True,
...
def normalize_openaq_observation_signals(
    ...
    return {
        ...
        "board_handoff": {
            ...
            "suggested_next_skills": ["query-environment-signals"],
        },
    }
