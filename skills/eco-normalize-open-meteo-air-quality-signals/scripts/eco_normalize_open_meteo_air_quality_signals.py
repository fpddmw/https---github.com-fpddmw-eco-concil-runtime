#!/usr/bin/env python3
"""Normalize open-meteo-air-quality-fetch artifacts into environment signal-plane rows."""
...
SKILL_NAME = "eco-normalize-open-meteo-air-quality-signals"
SOURCE_SKILL = "open-meteo-air-quality-fetch"
PLANE = "environment"
...
def build_signals(payload: Any, run_id: str, round_id: str, artifact_file: Path, artifact_sha256: str) -> tuple[list[dict[str, Any]], list[dict[str, str]]]:
    ...
    for record_index, record in enumerate(records):
        if not isinstance(record, dict):
            continue
        hourly = record.get("hourly") if isinstance(record.get("hourly"), dict) else {}
        units = record.get("hourly_units") if isinstance(record.get("hourly_units"), dict) else {}
        time_values = hourly.get("time") if isinstance(hourly.get("time"), list) else []
        ...
        for key, values in hourly.items():
            ...
            if len(values) != len(time_values):
                warnings.append({"code": "length-mismatch", "message": f"Skipped hourly.{key} because value count did not match time count."})
                continue
            ...
            for index, raw_value in enumerate(values):
                ...
                signals.append(
                    base_signal(
                        ...
                        latitude=latitude,
                        longitude=longitude,
                        quality_flags=["modelled-air-quality"],
                        engagement={},
                        metadata={"section": "hourly", "timezone": maybe_text(record.get("timezone"))},
                        raw_record={"time": observed_at, "metric": key, "value": raw_value},
                        artifact_path=artifact_file,
                        record_locator=f"$.records[{record_index}].hourly.{key}[{index}]",
                        artifact_sha256=artifact_sha256,
                    )
                )
    if not signals:
        warnings.append({"code": "no-signals", "message": "No Open-Meteo air-quality values produced normalized signals."})
    return signals, warnings


def normalize_open_meteo_air_quality(run_dir: str, run_id: str, round_id: str, artifact_path: str, db_path: str) -> dict[str, Any]:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Normalize open-meteo-air-quality-fetch artifacts into environment signal-plane rows.")
...
def main() -> int:
    args = parse_args()
    payload = normalize_open_meteo_air_quality(
