...
```bash
python3 scripts/fetch_youtube_comments.py check-config --pretty
```
...
- Accept upstream video IDs from CLI or from `$fetch-youtube-video-search` JSON/JSONL outputs.
...
- Invoke with `$fetch-youtube-comments`.
- Keep the skill atomic: each invocation consumes one configured ID set and time window.
- Prefer chaining from `$fetch-youtube-video-search` output via `--video-ids-file`.
