---
name: fetch-youtube-video-search
...
```bash
python3 scripts/fetch_youtube_video_search.py check-config --pretty
```
...
- Feed its JSONL output directly into `$fetch-youtube-comments` via `--video-ids-file`.
...
python3 scripts/fetch_youtube_video_search.py search \
  ...
  --output-dir [OUTPUT_DIR] \
  --pretty
Take artifacts.output_jsonl and pass it to $fetch-youtube-comments as --video-ids-file.
