...
```bash
python3 scripts/fetch_regulationsgov_comments.py check-config --pretty
```

2. Dry-run to verify query parameters without making remote calls.

```bash
python3 scripts/fetch_regulationsgov_comments.py fetch \
...
```text
Use $fetch-regulationsgov-comments.
Run:
python3 scripts/fetch_regulationsgov_comments.py fetch \
  --filter-mode last-modified \
