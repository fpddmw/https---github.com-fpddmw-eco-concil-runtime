#!/usr/bin/env python3
"""Normalize youtube-video-search artifacts into a local signal-plane SQLite file."""
...
SKILL_NAME = "eco-normalize-youtube-video-public-signals"
...
def public_gap_hints(signals: list[dict[str, Any]]) -> list[str]:
    ...
    hints: list[str] = []
    missing_descriptions = sum(1 for signal in signals if not maybe_text(signal.get("body_text")))
    if missing_descriptions:
        hints.append(f"{missing_descriptions} YouTube signals are missing a description field.")
    if len(signals) < 2:
...
def public_challenge_hints() -> list[str]:
    return ["Video metadata can describe commentary or compilation rather than original reporting."]
...
def build_signals(
    ...
    for index, item in enumerate(payload):
        ...
        video_id = maybe_text(video.get("id") or item.get("video_id"))
        title = maybe_text(video.get("title"))
        description = maybe_text(video.get("description"))
        signal_id = "sig-" + stable_hash(run_id, round_id, SOURCE_SKILL, artifact_sha256, video_id or index, title)[:16]
        signals.append(
            {
                "signal_id": signal_id,
                "run_id": run_id,
                ...
                "dedupe_key": video_id or title or str(index),
                "title": title,
                "body_text": description,
                "url": maybe_text(video.get("url")) or (f"https://www.youtube.com/watch?v={video_id}" if video_id else ""),
                "author_name": maybe_text(video.get("channel_title")),
                "channel_name": maybe_text(video.get("channel_title")),
                "language": maybe_text(video.get("default_language")),
                "query_text": maybe_text(query_text_override) or maybe_text(item.get("query")),
                "metric": "",
                "numeric_value": None,
                "unit": "",
                "published_at_utc": maybe_text(video.get("published_at")),
                "observed_at_utc": "",
                "window_start_utc": "",
                "window_end_utc": "",
                ...
                "engagement_json": json.dumps(video.get("statistics") if isinstance(video.get("statistics"), dict) else {}, ensure_ascii=True, sort_keys=True),
                "metadata_json": json.dumps({"video_id": video_id}, ensure_ascii=True, sort_keys=True),
    ...
    if not signals:
        warnings.append({"code": "no-signals", "message": "No YouTube results produced normalized signals."})
    return signals, warnings


def normalize_youtube_video_public_signals(
    run_dir: str,
    run_id: str,
    round_id: str,
    artifact_path: str,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Normalize youtube-video-search artifacts into a local signal-plane SQLite file.")
...
def main() -> int:
    args = parse_args()
    payload = normalize_youtube_video_public_signals(
