...
SKILL_NAME = "extract-observation-candidates"
...
def extract_observation_candidates_skill(
    ...
    return {
        ...
        "board_handoff": {"candidate_ids": [candidate["observation_id"] for candidate in candidates], "evidence_refs": artifact_refs[:20], "gap_hints": ["Observation candidates without stable coordinates still need spatial refinement."] if candidates else [], "challenge_hints": ["Check whether provider-specific clusters should be merged before matching."] if candidates else [], "suggested_next_skills": ["merge-observation-candidates", "build-normalization-audit"]},
    }
