#!/usr/bin/env python3
"""Deprecated WP4 alias for legacy claim/observation linking."""
...
SKILL_NAME = "link-claims-to-observations"
...
def deprecated_helper_payload(
    *,
    ...
    metadata = wp4_helper_metadata(
        ...
        rule_trace=["wp4-deprecated-alias-block"],
        caveats=[
            "Legacy claim/observation matching is isolated and no longer emits empirical judgement objects.",
            "Use a successor scope-review helper only after an agent submits an explicit verification question and scope.",
    ...
    wrapper = {
        ...
        "query_basis": {
            "run_dir": str(run_dir),
            "min_score_ignored": float(min_score),
            "top_links_per_claim_ignored": max(1, int(top_links_per_claim)),
            "method": "wp4-deprecated-alias-block",
        },
        "replacement": {
            "skill": "review-fact-check-evidence-scope",
            "required_explicit_inputs": [
                "verification_question",
                "geographic_scope",
                "study_period",
                "evidence_window",
                "propagation_or_lag_assumptions",
                "metric_requirements",
                "source_requirements",
            ],
        },
        ...
        "warnings": [
            {
                ...
                "message": "WP4 isolates this legacy helper; no empirical judgement objects were produced.",
            },
            {
                "code": "successor-helper-required",
                "message": "Submit an explicit scoped verification question and use the successor scope-review helper instead.",
            },
        ],
    }
    ...
    return {
        ...
        "board_handoff": {
            ...
            "challenge_hints": [
                "Review the requested scope, taxonomy, source coverage, and report usage before approving any successor helper."
            ],
...
def link_claims_to_observations_skill(
    ...
    output_file = resolve_path(
        run_dir_path,
        output_path,
        f"claim_observation_links_{round_id}.json",
    )
    return deprecated_helper_payload(
        ...
        min_score=min_score,
        top_links_per_claim=top_links_per_claim,
    )


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Deprecated WP4 alias for legacy claim/observation linking."
    )
