...
```bash
python3 scripts/fetch_open_meteo_air_quality.py check-config --pretty
```
...
python3 scripts/fetch_open_meteo_air_quality.py fetch \
  ...
  --timezone GMT \
  --output ./runs/manual-fetch-artifacts/open-meteo/fetch-open-meteo-air-quality.json \
  --log-level INFO \
  --log-file ./logs/fetch-open-meteo-air-quality.log \
  --pretty
```
