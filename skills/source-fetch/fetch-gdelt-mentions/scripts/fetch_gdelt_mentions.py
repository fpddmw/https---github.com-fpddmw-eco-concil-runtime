...
DEFAULT_USER_AGENT = "fetch-gdelt-mentions/1.0"
...
def build_logger(level: str, log_file: str) -> logging.Logger:
    logger = logging.getLogger("fetch-gdelt-mentions")
...
def collect_masterfile_mentions(
    *,
    url: str,
    start: datetime,
    end: datetime,
    ...
    for attempt in range(1, attempts + 1):
        ...
        try:
            with request.urlopen(req, timeout=client._cfg.timeout_seconds) as resp:
                ...
                candidates = sorted(
                    iter_masterfile_mentions(lines=lines, start=start, end=end, logger=logger),
...
def select_entries(args: argparse.Namespace, config: RuntimeConfig, logger: logging.Logger) -> list[ExportFileEntry]:
    ...
    candidates = collect_masterfile_mentions(
...
def build_parser() -> argparse.ArgumentParser:
    ...
    fetch.add_argument(
        "--output-dir",
        default="./runs/manual-fetch-artifacts/gdelt-mentions",
