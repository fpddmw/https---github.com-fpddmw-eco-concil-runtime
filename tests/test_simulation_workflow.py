...
from eco_council_runtime.application.simulation_workflow import (  # noqa: E402
...
class SimulationWorkflowTests(unittest.TestCase):
    def test_command_list_presets_reports_builtins(self) -> None:
    ...
    def test_simulate_round_writes_execution_and_dependency_artifacts(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertTrue(Path(payload["execution_path"]).exists())
            video_artifact = Path(payload["statuses"][0]["artifact_path"])
            comment_artifact = Path(payload["statuses"][1]["artifact_path"])
            self.assertTrue(video_artifact.exists())
            self.assertTrue(comment_artifact.exists())
            self.assertGreaterEqual(len(video_artifact.read_text(encoding="utf-8").splitlines()), 1)
            self.assertGreaterEqual(len(comment_artifact.read_text(encoding="utf-8").splitlines()), 1)
