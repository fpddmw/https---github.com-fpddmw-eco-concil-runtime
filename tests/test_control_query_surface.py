...
def seed_control_state(run_dir: Path) -> dict[str, str]:
    ...
    gate_path = (run_dir / "runtime" / f"report_basis_gate_{ROUND_ID}.json").resolve()
    ...
    plan_payload = {
        ...
        "downstream_posture": "report-basis-candidate",
        "assigned_role_hints": ["moderator"],
        "phase_decision_basis": {
            "probe_stage_reason_codes": [],
            "posture_reason_codes": ["report-basis-ready"],
        },
        "agent_turn_hints": {
        ...
        "execution_queue": [
            {
                ...
                "reason": "Readiness review is required before report-basis freeze gate.",
        ...
        "gate_steps": [
            {
                ...
                "blocking": True,
                "resume_policy": "skip-if-completed",
                "operator_summary": "Evaluate whether the round can move into report-basis freeze.",
        ...
        "post_gate_steps": [
            {
                ...
                "blocking": True,
                "resume_policy": "skip-if-completed",
                "operator_summary": "Freeze the frozen evidence basis.",
                "reason": "Persist the final report_basis.",
                "expected_output_path": str(
                    (run_dir / "report_basis" / f"frozen_report_basis_{ROUND_ID}.json").resolve()
                ),
            }
        ],
        "stop_conditions": [
    ...
    controller_snapshot = {
        ...
        "artifacts": {
            ...
            "report_basis_gate_path": str(gate_path),
    ...
    supervisor_snapshot = {
        ...
        "top_actions": [],
        "operator_notes": [
            "Report basis freeze succeeded and the evidence basis is now ready for downstream reporting."
        ],
        "inspection_paths": {
            "controller_path": str(controller_path),
            "plan_path": str(
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            try:
                with connection:
                    ...
                    gate_payload["gate_status"] = "report-basis-freeze-withheld"
                    ...
                    freeze_payload["report_basis_status"] = "withheld"
                    ...
                    connection.execute(
                        "UPDATE report_basis_freezes SET raw_json = ? WHERE freeze_id = ?",
                        (
                            json.dumps(
                                freeze_payload,
                                ensure_ascii=True,
            ...
            self.assertEqual("frozen", controller["report_basis_status"])
            self.assertEqual(
            ...
            self.assertEqual("report-basis-gate", gate["stage_name"])
            self.assertEqual("report-basis-gate", gate["gate_handler"])
            self.assertEqual("report-basis-freeze-allowed", gate["gate_status"])
            ...
            self.assertEqual("frozen", freeze["report_basis_status"])
