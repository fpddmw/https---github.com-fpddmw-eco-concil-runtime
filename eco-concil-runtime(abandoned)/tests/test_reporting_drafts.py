...
class ReportingDraftsTests(unittest.TestCase):
    ...
    def test_build_investigation_review_draft_prefers_investigation_actions(self) -> None:
        mission = example_mission(run_id="drafts-review-actions-run")
        state = {
            "mission": mission,
            "round_id": ROUND_ID,
            "matching_authorization": {"authorization_id": "auth-001"},
    ...
    def test_build_expert_report_draft_prefers_investigation_actions_before_readiness_fallback(self) -> None:
        mission = example_mission(run_id="drafts-report-actions-run")
        state = {
            "mission": mission,
            "round_id": ROUND_ID,
            "claims": [
    ...
    def test_build_decision_draft_from_state_uses_investigation_actions_for_task_order(self) -> None:
        mission = example_mission(run_id="drafts-decision-actions-run")
        state = {
            "mission": mission,
            "round_id": ROUND_ID,
            "claims": [
    ...
    def test_build_investigation_review_draft_surfaces_governed_probe_requests(self) -> None:
        state = competing_hypothesis_state(run_id="drafts-probe-review-run")
    ...
    def test_build_decision_draft_carries_probe_requests_from_review(self) -> None:
        state = competing_hypothesis_state(run_id="drafts-probe-decision-run")
        state["investigation_actions"] = {
            "ranked_actions": [
                {
                    "assigned_role": "environmentalist",
    ...
    def test_build_investigation_review_draft_surfaces_competing_hypothesis_gating(self) -> None:
        payload = build_investigation_review_draft_from_state(
            competing_hypothesis_state(run_id="drafts-review-gating-run")
        )
    ...
    def test_build_decision_draft_from_state_uses_review_gating_without_remands(self) -> None:
        state = competing_hypothesis_state(run_id="drafts-decision-gating-run")
        state["investigation_review"] = build_investigation_review_draft_from_state(state)
