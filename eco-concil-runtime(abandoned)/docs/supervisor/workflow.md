...
- `awaiting-moderator-task-review`
  ...
  - Manual fallback: use the provisioned moderator agent workspace plus the current task-review outbox prompt, then use `import-task-review`.
- `awaiting-source-selection`
  - Prefer `run-agent-step --role sociologist` and `run-agent-step --role environmentalist`.
  - Manual fallback: use the two provisioned expert agent workspaces plus the current source-selection outbox prompts, then use `import-source-selection`.
...
- `awaiting-expert-reports`
  - Prefer `run-agent-step --role sociologist` and `run-agent-step --role environmentalist`.
  - Manual fallback: use the two provisioned expert agent workspaces plus the current report outbox prompts, then use `import-report`.
- `awaiting-moderator-decision`
  - Prefer `run-agent-step`.
  - Manual fallback: use the provisioned moderator agent workspace plus the current decision outbox prompt, then use `import-decision`.
- `ready-to-promote`
  - Run `continue-run`.
...
- Provisioning also projects the detached eco-council skill directories into the isolated OpenClaw `managedSkillsDir`, so role agents can use repository skills without assuming this runtime lives inside the same checkout.
...
- The signal-corpus CLI resolves analytics DB paths from `RUN_DIR/run_manifest.json` first and otherwise falls back to `RUN_DIR/analytics/public_signals.sqlite` plus `RUN_DIR/analytics/environment_signals.sqlite`.
