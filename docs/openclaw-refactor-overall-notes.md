...
1. 历史文档和部分测试文案仍有 `coverage / promotion / contradiction` 等旧词汇；当前不构成默认链路风险，但会影响后续维护者理解。
2. `promote-evidence-basis` 仍作为兼容 skill id 存在；语义已收缩为 `report-basis-freeze`，但命名债需要单独迁移。
