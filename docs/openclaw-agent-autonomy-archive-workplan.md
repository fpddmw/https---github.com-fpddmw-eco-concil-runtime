...
9. source-acquisition proposal 增加薄生命周期更新入口：`update-source-acquisition-proposal-status` 只记录 `proposed|approved-for-execution|executed|withdrawn|rejected` 状态、证据 ref、lineage 和 provenance，不执行 source 排序、不判断证据采信；agent entry、operator view、source intent surface 均提供对应命令模板。
