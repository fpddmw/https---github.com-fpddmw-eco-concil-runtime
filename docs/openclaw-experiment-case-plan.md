...
文档性质：本文只记录毕业设计展示用实验/案例安排，不规定代码开发任务，不作为 runtime 议程脚本，也不要求 agent 固定使用某个 source。代码层架构基线见 `docs/openclaw-project-overview.md`；三案例补强和重跑审计见 `docs/diagnostics/openclaw-case-rerun-and-skill-uptake-audit.md`。
