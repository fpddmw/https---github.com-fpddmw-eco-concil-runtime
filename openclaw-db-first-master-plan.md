...
| `A2` | Shared Contract Hardening | 统一 `analysis_sync / deliberation_sync / observed_inputs / *_source` 等共享输出契约 | `planned` | 关键技能输出字段稳定且不再重复漂移 |
...
| `B2` | Board Write-Path Migration | board 状态变更从“JSON first, DB sync”转向“DB first, JSON export” | `in_progress` | 关键 state-change skills 的主写面切到 deliberation plane |
...
| `C2` | Generic Result-Set Contract | 强化 `result_sets / result_items` 的通用契约与 lineage 语义 | `planned` | result set 可追溯 query basis、parent ids、artifact refs |
| `C2.1` | Candidate / Cluster Result Migration | 将 claim/observation candidate、cluster、merge 等对象纳入 analysis plane | `planned` | 早期分析链的关键压缩对象可被统一查询 |
...
2. `B` 路线已经完成 deliberation-plane 读路径迁移，并完成核心 note / hypothesis / challenge / task mutation 的 DB-first 切换；剩余 round transition / open-round 写路径仍待继续切主。
...
| --- | --- | --- | --- | --- |
| `1` | `B2` | `B` | B/C 两条读路径迁移已基本完成，下一步最有价值的是切 board 主写面 | board state-change skills 变成 DB-first primary |
| `2` | `B2.1` | `B` | 一旦主写面切换，就应把 summary/brief 真正降级为导出物 | board exports 不再是运营前置 |
| `3` | `A2` | `A` | 在 B/C 路线大迁移后统一 contract，避免 trace 字段再次漂移 | 共享输出契约文档和代码硬化 |
| `4` | `C2` | `C` | 当前 analysis plane 已覆盖更多结果集，下一步应补强通用 result-set / lineage 契约 | result set 可追溯 query basis、parent ids、artifact refs |
| `5` | `D3` | `D` | 路线越来越多，需要更强的阶段看板和阻塞视图 | progress dashboard 约束与汇总模板 |
