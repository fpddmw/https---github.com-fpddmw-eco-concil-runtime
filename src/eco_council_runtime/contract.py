...
ASSETS_DIR = CONTRACT_ASSETS_DIR
EXAMPLES_DIR = CONTRACT_EXAMPLES_DIR
...
INVESTIGATION_REVIEW_STATUSES = {"supported", "partial", "unresolved", "blocked"}
INVESTIGATION_LEG_STATUSES = {"supported", "partial", "unresolved", "contradicted", "isolated"}
...
def validate_investigation_leg_review(value: Any, path: str, issues: IssueCollector) -> None:
    record = require_object(value, path, issues)
    if record is None:
        return
    require_string(record, "leg_id", path, issues)
    require_enum(record, "status", path, issues, allowed=INVESTIGATION_LEG_STATUSES)
    require_string(record, "summary", path, issues)
    evidence_refs = validate_string_list(record.get("evidence_refs"), f"{path}.evidence_refs", issues)
    validate_unique_strings(evidence_refs, f"{path}.evidence_refs", issues)
    notes = validate_string_list(record.get("notes"), f"{path}.notes", issues)
    validate_unique_strings(notes, f"{path}.notes", issues)


def validate_investigation_hypothesis_review(value: Any, path: str, issues: IssueCollector) -> None:
    record = require_object(value, path, issues)
    if record is None:
        return
    require_string(record, "hypothesis_id", path, issues)
    require_string(record, "statement", path, issues)
    require_enum(record, "overall_status", path, issues, allowed=INVESTIGATION_REVIEW_STATUSES)
    for field_name in ("matched_card_ids", "isolated_entry_ids", "remand_ids", "notes"):
        values = validate_string_list(record.get(field_name), f"{path}.{field_name}", issues)
        validate_unique_strings(values, f"{path}.{field_name}", issues)
    leg_reviews = record.get("leg_reviews")
    if not isinstance(leg_reviews, list):
        issues.add(f"{path}.leg_reviews", "Expected a list.", actual=leg_reviews)
    else:
        for index, leg_review in enumerate(leg_reviews):
            validate_investigation_leg_review(leg_review, f"{path}.leg_reviews[{index}]", issues)


def validate_investigation_review_object(obj: Any, path: str, issues: IssueCollector) -> None:
    record = require_object(obj, path, issues)
    if record is None:
        return
    validate_schema_version(record, path, issues)
    require_string(record, "review_id", path, issues)
    require_string(record, "run_id", path, issues)
    require_string(record, "round_id", path, issues)
    role = require_enum(record, "agent_role", path, issues, allowed=AGENT_ROLES)
    if role is not None and role != "moderator":
        issues.add(f"{path}.agent_role", "investigation-review must be moderator-owned.", actual=role)
    require_string(record, "authorization_id", path, issues)
    require_string(record, "matching_result_id", path, issues)
    require_enum(record, "review_status", path, issues, allowed=INVESTIGATION_REVIEW_STATUSES)
    require_bool(record, "matching_reasonable", path, issues)
    require_bool(record, "needs_additional_data", path, issues)
    require_string(record, "summary", path, issues)
    for field_name in ("matched_card_ids", "isolated_entry_ids", "remand_ids", "open_questions"):
        values = validate_string_list(record.get(field_name), f"{path}.{field_name}", issues)
        validate_unique_strings(values, f"{path}.{field_name}", issues)
    hypothesis_reviews = record.get("hypothesis_reviews")
    if not isinstance(hypothesis_reviews, list):
        issues.add(f"{path}.hypothesis_reviews", "Expected a list.", actual=hypothesis_reviews)
    else:
        for index, hypothesis_review in enumerate(hypothesis_reviews):
            validate_investigation_hypothesis_review(
                hypothesis_review,
                f"{path}.hypothesis_reviews[{index}]",
                issues,
            )
    recommendations = record.get("recommended_next_actions")
    if not isinstance(recommendations, list):
...
VALIDATORS = {
    ...
    "investigation-review": validate_investigation_review_object,
...
EXAMPLES: dict[str, Any] = {
    ...
    "investigation-review": read_json(EXAMPLES_DIR / "investigation_review.json"),
...
def _contract_runtime():
    from eco_council_runtime.application import contract_runtime as application_contract_runtime

    return application_contract_runtime


def load_ddl() -> str:
    return _contract_runtime().load_ddl()


def parse_point(raw: str) -> dict[str, Any]:
    return _contract_runtime().parse_point(raw)


def parse_bbox(raw: str) -> dict[str, Any]:
    return _contract_runtime().parse_bbox(raw)


def allowed_sources_for_role(mission: dict[str, Any], role: str) -> list[str]:
    return _contract_runtime().allowed_sources_for_role(mission, role)


def expected_output_kinds_for_role(role: str) -> list[str]:
    return _contract_runtime().expected_output_kinds_for_role(role)


def investigation_leg_ids(mission: dict[str, Any], round_id: str) -> set[str]:
    return _contract_runtime().investigation_leg_ids(mission, round_id)


def default_public_claim_summary(leg_ids: set[str]) -> str:
    return _contract_runtime().default_public_claim_summary(leg_ids)


def default_environment_summary(leg_ids: set[str]) -> str:
    return _contract_runtime().default_environment_summary(leg_ids)


def default_round_tasks(*, mission: dict[str, Any], round_id: str) -> list[dict[str, Any]]:
    return _contract_runtime().default_round_tasks(mission=mission, round_id=round_id)
...
def placeholder_source_selection(
    *,
    ...
    return _contract_runtime().placeholder_source_selection(
        run_id=run_id,
        round_id=round_id,
        role=role,
        task_ids=task_ids,
...
def placeholder_claim_curation(*, run_id: str, round_id: str) -> dict[str, Any]:
    return _contract_runtime().placeholder_claim_curation(run_id=run_id, round_id=round_id)


def placeholder_observation_curation(*, run_id: str, round_id: str) -> dict[str, Any]:
    return _contract_runtime().placeholder_observation_curation(run_id=run_id, round_id=round_id)


def placeholder_report(*, run_id: str, round_id: str, role: str) -> dict[str, Any]:
    return _contract_runtime().placeholder_report(run_id=run_id, round_id=round_id, role=role)


def placeholder_investigation_plan(*, run_id: str, round_id: str) -> dict[str, Any]:
    return _contract_runtime().placeholder_investigation_plan(run_id=run_id, round_id=round_id)
...
def normalize_round_tasks(
    *,
    ...
    return _contract_runtime().normalize_round_tasks(tasks=tasks, run_id=run_id, round_id=round_id)
...
def scaffold_round(
    *,
    ...
    return _contract_runtime().scaffold_round(
        run_dir=run_dir,
        run_id=run_id,
        round_id=round_id,
        tasks=tasks,
...
def scaffold_run_from_mission(
    *,
    ...
    return _contract_runtime().scaffold_run_from_mission(
        run_dir=run_dir,
        mission=mission,
        tasks=tasks,
        pretty=pretty,
...
def scaffold_run(
    *,
    ...
    return _contract_runtime().scaffold_run(
        run_dir=run_dir,
        run_id=run_id,
        topic=topic,
        objective=objective,
...
def validate_bundle(run_dir: Path) -> dict[str, Any]:
    return _contract_runtime().validate_bundle(run_dir)


def command_list_kinds(args: argparse.Namespace) -> dict[str, Any]:
    return _contract_runtime().command_list_kinds(args)


def command_write_example(args: argparse.Namespace) -> dict[str, Any]:
    return _contract_runtime().command_write_example(args)


def command_validate(args: argparse.Namespace) -> dict[str, Any]:
    return _contract_runtime().command_validate(args)


def command_init_db(args: argparse.Namespace) -> dict[str, Any]:
    return _contract_runtime().command_init_db(args)


def command_scaffold_run(args: argparse.Namespace) -> dict[str, Any]:
    return _contract_runtime().command_scaffold_run(args)


def command_scaffold_run_from_mission(args: argparse.Namespace) -> dict[str, Any]:
    return _contract_runtime().command_scaffold_run_from_mission(args)


def command_scaffold_round(args: argparse.Namespace) -> dict[str, Any]:
    return _contract_runtime().command_scaffold_round(args)


def command_validate_bundle(args: argparse.Namespace) -> dict[str, Any]:
    return _contract_runtime().command_validate_bundle(args)


def _contract_cli():
    from eco_council_runtime.cli import contract_cli as runtime_contract_cli

    return runtime_contract_cli


def add_pretty_flag(parser: argparse.ArgumentParser) -> None:
    _contract_cli().add_pretty_flag(parser)


def build_parser() -> argparse.ArgumentParser:
    return _contract_cli().build_contract_parser()


def main(argv: list[str] | None = None) -> int:
    return _contract_cli().run_contract_cli(argv)
