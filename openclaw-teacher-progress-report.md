...
同时，如果后续要保留 runtime source-queue 模式，也应把它保留为第二套编排流程，而不是单独维护另一套 skill 体系。skills 必须继续与编排层解耦。
...
另外，当前活跃 runtime 的 source queue 也还不能说“即将可以投产”。当前 prepare/import 仍只是最小本地 import 闭环，不是真正的 live source-selection 和 external fetch execution。
...
有关当前 46 个活跃 skill 的双流程适配结论，见 [openclaw-dual-flow-skill-assessment.md](openclaw-dual-flow-skill-assessment.md)。
