    ...
    overview_path.write_text(
        "\n".join(
            [
                ...
                "",
                "- For council evidence acquisition, use repo-integrated runtime skills exposed in `role_surface.json`; platform-level `web_search`, `web_fetch`, or browser tools are prohibited as evidence-acquisition routes.",
                "- Treat `skills_by_layer`, `fetch_command_surfaces`, and `source_family_workflows` in `role_surface.json` as the authoritative local skill inventory for this role.",
                "- Platform `web_search`, `web_fetch`, or browser tools are not OpenClaw source-acquisition records; do not use them to replace an available runtime skill, create formal evidence, or support a negative evidence claim.",
                "- If no local skill route fits the evidence need, submit an evidence-route-assessment naming the unavailable route or capability gap instead of using platform web tools.",
