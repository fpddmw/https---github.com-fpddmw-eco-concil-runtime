...
OPTIONAL_ANALYSIS_HELPER_FREEZE_LINES: dict[str, dict[str, Any]] = {
    ...
    "materialize-acquisition-checkpoints": {
        "rule_id": "HEUR-ACQUISITION-CHECKPOINT-001",
        "destination": "claim-impact in-round acquisition checkpoints",
        "caveats": [
            "Checkpoints are advisory and recorded only when acquisition state affects claim strength, source-limit rationale, downgrade wording, or recovery choice.",
            "Checkpoint output is not a form for every tool call and cannot prove claim truth, rank routes, or replace findings.",
        ],
    },
    "review-theme-sufficiency": {
...
POLICIES.update(
    {
        ...
        "materialize-report-blueprint": _policy(
        ...
        "draft-agent-section-brief": _policy(
            ...
            output_object_kinds=["agent-section-brief"],
            write_scope=WRITE_SCOPE_REPORTING,
            default_actor_role_hint=ROLE_SOCIAL_INVESTIGATOR,
        ),
